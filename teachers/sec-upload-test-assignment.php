...
if (is_uploaded_file($_FILES['img']['tmp_name']) AND $_FILES['img']['error'] == UPLOAD_ERR_OK){ 
	...
	if (empty($errors)){
		...
		$new_name= (string) sha1($_FILES['img']['name'] . uniqid('',true));
			...
			$dest = "../../incs-storage/sec-assignments/".$new_name;
...
mysqli_query($connect, "INSERT INTO secondary_test_assignment_upload (secondary_test_upload_class_id, secondary_test_upload_filename, secondary_test_upload_testname) VALUES ('".$_SESSION['secondary_teacher_class_id']."', '".$new_name."', '".$assignment_name."')") or die(db_conn_error);
